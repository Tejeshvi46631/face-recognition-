...
// Select Designation API Call
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        List<String> flanamelist =
            jsonResponse.map((item) => item['empname'].toString()).toList();
  ...
  static Future<void> registration({
    ...
    var requestUrl = apiUrl.replace(queryParameters: {
      ...
      "fla": isFLA ? "true" : "false",
      "empid": employeeId,
      "responsecode": "1",
      "reportingofficer": selectedflaname,
      "is_verify": "true",
      "is_guidelines_accepted": "true",
    });
    try {
      final response = await http.post(
        requestUrl,
  ...
  static Future<void> insertData(String name, List<double> embedding, String empid) async {
    print("embdedding----------------------------------");
    print(embedding);
  ...
  static Future<void> insertDataMap(String name, List<double> embedding, String empid) async {
    // Convert the data object to JSON
  ...
  static Future<Map<String, dynamic>> insertDataName(String name, List<double> embedding, String empid) async {

    String emd = embedding.join(',');

    //Map<String, dynamic> jsonResponse=[];
    var url = Uri.parse('http://164.100.112.121/GeoFenceAPI/getByEmpId?name='+name+'?embedding'+emd+'?empid='+empid);
  ...
  static Future<Map<String, dynamic>> getAllByEmpId(String empid) async {
    ...
    //Map<String, dynamic> jsonResponse=[];
    var url = Uri.parse('http://164.100.112.121/GeoFenceAPI/getByEmpId?empid='+empid);
